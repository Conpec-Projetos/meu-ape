...
export async function buscarPropriedadesPaginado(
    ...
    try {
        let q = query(collection(db, "properties"), orderBy("criadoEm", "desc"), limit(pageSize));

        if (lastDoc) {
            q = query(collection(db, "properties"), orderBy("criadoEm", "desc"), startAfter(lastDoc), limit(pageSize));
        }
