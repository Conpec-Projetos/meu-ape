CREATE OR REPLACE FUNCTION search_properties(
    ...
    bedroom_filter INT[] DEFAULT NULL, -- Array de inteiros
...
RETURNS TABLE (
    ...
    total_matching_units BIGINT, -- Contagem de unidades que atendem aos filtros
    total_properties BIGINT -- Contagem total de propriedades
)
AS $$
DECLARE
...
BEGIN
    ...
    AND
        (min_lat IS NULL OR ST_Intersects(
            p.location::geometry,
    ...
    GROUP BY
        p.id
    ORDER BY
        p.launch_date DESC -- Ordenação para consistência da paginação
    LIMIT page_limit
    OFFSET page_offset;
END;
$$ LANGUAGE plpgsql;