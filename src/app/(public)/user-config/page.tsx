...
export default function UserConfig() {
    ...
    const DocumentRow: React.FC<{ label: string; typeKey: string; uploadedUrls: string[]; pendingFile?: File | null; setPendingFile: (f: File | null) => void }> = ({ label, typeKey, uploadedUrls, pendingFile, setPendingFile }) => {
        const inputId = `doc-${typeKey}`;
        return (
            <div className="gap-4">
                <h1 className="flex flex-row pb-1 text-2xl text-[#4D4D4D]">{label}: 
                <div className="ml-2">
                    {uploadedUrls.map((u, i) => (
                        <a key={i} href={u} target="_blank" rel="noreferrer" className=" text-sm text-blue-600 underline">Ver {label}</a>
                    ))}
                    {pendingFile && <span className="text-sm text-gray-600">Arquivo pronto para upload: {pendingFile.name}</span>}
                </div>
                </h1>
                <label htmlFor={inputId} className="relative cursor-pointer ml-4 p-6 pt-1 text-gray-400 border-gray-400 border-3 border-dashed rounded-[10px] h-27 w-27 flex items-center justify-center">
                    <DownloadIcon className="absolute top-1/3" />
                </label>
                <input id={inputId} type="file" className="hidden" onChange={(e) => setPendingFile(e.target.files ? e.target.files[0] : null)} />
            </div>
        );
    };
        ...
        try {
            ...
            if (!res.ok) {
                ...
                if (body?.issues && Array.isArray(body.issues)) {
                    const issues: Record<string, string> = {};
                    body.issues.forEach((it: any) => {
                        if (it.field) issues[it.field] = it.message || it.error || 'Inválido';
                    });
                    setFormErrors(issues);
                }
                throw new Error(body.error || `HTTP ${res.status}`);
    ...
    const handleSaveDocuments = async () => {
        ...
        try {
            ...
            setError(err?.message || 'Erro ao salvar documentos');
        } finally {
            setUploading(false);
        }
    };
            ...
            <div className="flex flex-col justify-center items-center pt-2">
                        ...
                        <div className="">
                            <div className="pl-15 pt-6">
                                <div className="flex flex-col items-start pt-2">
                                    ...
                                    <Input type="text" value={name} onChange={(e) => setName(e.currentTarget.value)} className=" border-1 border-gray-400 pl-5 rounded-[10px] bg-white w-[340px] h-[43px]"></Input>
                                    {formErrors.fullName && <p className="text-red-500 text-sm mt-1">{formErrors.fullName}</p>}
                                </div>
                                <div className="flex flex-col items-start pt-4">
                                    <h1 className="text-[#4D4D4D] text-[12px]">Endereço</h1>
                                    <Input value={address} onChange={(e) => setAddress(e.currentTarget.value)} className=" border-1 border-gray-400 pl-5 rounded-[10px] bg-white w-[340px] h-[43px]"></Input>
                                    {formErrors.address && <p className="text-red-500 text-sm mt-1">{formErrors.address}</p>}
                                </div>
                                <div className="flex flex-col items-start pt-4">
                                    <h1 className="text-[#4D4D4D] text-[12px]">Email</h1>
                                    <Input value={email} onChange={(e) => setEmail(e.currentTarget.value)} className=" border-1 border-gray-400 pl-5 rounded-[10px] bg-white w-[340px] h-[43px]"></Input>
                                </div>
                                <div className="flex flex-col items-start pt-4">
                                    <h1 className="text-[#4D4D4D] text-[12px]">CPF</h1>
                                    <Input value={cpf} onChange={(e) => setCpf(formatCPF(e.currentTarget.value))} className=" border-1 border-gray-400 pl-5 rounded-[10px] bg-white w-[340px] h-[43px]"></Input>
                                    {formErrors.cpf && <p className="text-red-500 text-sm mt-1">{formErrors.cpf}</p>}
                                </div>
                                <div className="flex flex-col items-start pt-4">
                                    <h1 className="text-[#4D4D4D] text-[12px]">Telefone</h1>
                                    <Input value={phone} onChange={(e) => setPhone(formatPhone(e.currentTarget.value))} className=" border-1 border-gray-400 pl-5 rounded-[10px] bg-white w-[340px] h-[43px]"></Input>
                                    ...
                                    <DialogContent className="fixed top-1/2 left-1/2 z-50 w-[468px] h-[401px] max-w-md -translate-x-[50%] -translate-y-2/3 rounded-xl bg-blend-hard-light p-6 shadow-accent-foreground">
                                        <div className="mt-4 flex flex-col gap-y-4 items-center">
                                            <div>
                                                ...
                                                <Input
                                                    className="border-1 border-gray-400 pl-5 rounded-[10px] bg-white w-[340px] h-[43px]"
                                                    type="password"
                                                />
                                            </div>
                                            <div>
                            ...
                            <div className="pl-15 flex items-center justify-start flex-col">
                                <Button disabled={loading} onClick={handleSave} className="mt-2 mb-3 bg-black w-[340px] h-[50px] hover:bg-blend-color-burn cursor-pointer rounded-full">
                ...
                {activeTab === "documentos" && (
                    <div className="flex flex-col gap-5 pl-20 items-start justify-center h-full w-full">
                        <DocumentRow label="RG ou CIN" typeKey="rg" uploadedUrls={uploadedUrls["rg"] || []} pendingFile={pendingFiles["rg"] || null} setPendingFile={(f) => setPendingFile("rg", f)} />
                        <DocumentRow label="Comprovante de endereço" typeKey="addressProof" uploadedUrls={uploadedUrls["addressProof"] || []} pendingFile={pendingFiles["addressProof"] || null} setPendingFile={(f) => setPendingFile("addressProof", f)} />
                        <DocumentRow label="Comprovante de renda" typeKey="incomeProof" uploadedUrls={uploadedUrls["incomeProof"] || []} pendingFile={pendingFiles["incomeProof"] || null} setPendingFile={(f) => setPendingFile("incomeProof", f)} />
                        <DocumentRow label="Certidão de casamento" typeKey="marriageCert" uploadedUrls={uploadedUrls["marriageCert"] || []} pendingFile={pendingFiles["marriageCert"] || null} setPendingFile={(f) => setPendingFile("marriageCert", f)} />
                        <div className="flex items-center justify-center w-screen h-auto pb-2">
                            <Button disabled={uploading} onClick={handleSaveDocuments} className="bg-black w-[100px] hover:bg-blend-color-burn cursor-pointer ">
