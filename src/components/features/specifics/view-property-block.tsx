...
export default function Unidade({ tipo = "middle" }: UnidadeProps) {
    ...
    return (
        <div className={`h-14 w-full ${borderClass} flex justify-between items-center bg-secondary border-white`}>
            <div className="rounded-4xl w-6 ml-4 h-6 border-2 border-primary"></div>
            <Image src={purplearrow} alt="seta roxa" width={15} height={10} className="mr-4" />
        </div>
    );
}
