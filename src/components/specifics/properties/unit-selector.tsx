...
export function UnitSelector({ structure, onSelect, selectedBlock, selectedCategory }: UnitSelectorProps) {
        ...
        <div className="w-full space-y-2">
            {Object.entries(structure).map(([block, categories]) => (
                <Collapsible
                    ...
                    onOpenChange={() => setOpenBlock(openBlock === block ? null : block)}
                ...
                >
                    <CollapsibleTrigger className="cursor-pointer flex justify-between items-center w-full p-4 font-medium bg-secondary/50 hover:bg-secondary/80 transition-colors">
                        {block}
                        <ChevronDown
                            className={`h-5 w-5 transition-transform duration-200 ${openBlock === block ? "rotate-180" : ""}`}
                        />
                    </CollapsibleTrigger>
